...
//            l3.clear();
    ...
    public static void searchConstituencyByIndex ( int index){
        ...
        int notarow = rowData.size()-1;
        String notavotes = (driver.findElement(By.xpath("//*[@id='div1']/table[1]/tbody/tr[" + notarow + "]/td[6]")).getText());
        Integer notaintvote = Integer.parseInt(notavotes);
        int notavotes1 = notaintvote.intValue();
        l1.add(notavotes1);
        ...
        if(l1.size()== cs.size()-1) {
            System.out.println("Total count of candidate who have got less vote than NOTA is :" + c);
            c=0;
            if(l1.size()==70)
            {
                System.out.println("Total number of candidate who got less vote tha NOTA are : "+totalcount);
            }
            l1.clear();
        }
